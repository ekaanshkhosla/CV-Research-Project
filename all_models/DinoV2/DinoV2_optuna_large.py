...
def print_hyperparameters(image_size, batch_size, learning_rate, fc_units, dropout_rate):
...
def define_model(fc_units, dropout_rate):
...
storage_url = "sqlite:///CV_DinoV2_optuna_large3.db"
study_name = "CV_DinoV2_optuna_large3"
...
def objective(trial):
    ...
    # Hyperparameters
    image_size = trial.suggest_categorical('image_size', [112, 224])
    ...
    print_hyperparameters(image_size, batch_size, learning_rate, fc_units, dropout_rate)
    ...
    model = define_model(fc_units, dropout_rate)
    model.to(device)
